...
struct StreamingView: View {
    ...
    var body: some View{
        ...
        .onAppear {
            HTTPRequestClient.Builder()
                ...
                .setHeaders(
                    [
                        "dev-id": Bundle.main.object(forInfoDictionaryKey: "dev-id") as? String ?? "",
                        "x-api-key": Bundle.main.object(forInfoDictionaryKey: "x-api-key") as? String ?? ""
                    ]
                )
                .build()
                .executeAndGetResult { resp in
                    if let resp_ = resp, let token = resp_.token {
                        ...
                        display(hearBeat: 60, delay: 0.2)
                        display(hearBeat: 65, delay: 1.2)
                        display(hearBeat: 64, delay: 2.2)
                        display(hearBeat: 60, delay: 3.2)
                        display(hearBeat: 67, delay: 4.2)
                        display(hearBeat: 68, delay: 5.2)
                        display(hearBeat: 69, delay: 6.2)
                        display(hearBeat: 70, delay: 8.2)
                        display(hearBeat: 75, delay: 9.2)
                        display(hearBeat: 83, delay: 10.2)
                        display(hearBeat: 89, delay: 11.2)
                        display(hearBeat: 95, delay: 12.2)
                        display(hearBeat: 99, delay: 13.2)
                        display(hearBeat: 103, delay: 14.2)
                        
                        ContentView.terraRT?.startRealtime(
                            type: connectionType,
                            dataType: [.HEART_RATE, .ACCELERATION],
                            token: token
                        ) { update in
                            print(">>>Update 2 ", update)
                            if let hr = update.val, let type = update.type, type.contains("HEART_RATE"){
    ...
    private func start() {
        HTTPRequestClient.Builder()
            ...
            .executeAndGetResult { resp in
                if let resp_ = resp, let token = resp_.token {
                    ContentView.terraRT?.startRealtime(type: connectionType, dataType: [
                        .HEART_RATE,
                        .ECG,
                        .STEPS,
                        .HRV,
    ...
    private func display(hearBeat: Double, delay: TimeInterval) {
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
            self.hrPoints.append(Datapoint(y: hearBeat, x: Date()))
            if self.hrPoints.count > 11{
                self.hrPoints.removeFirst()
            }
            isPulsing = true
